  ...
  return (
    <Stack spacing={2}>
      <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
        <Typography variant="h6" sx={{ fontWeight: 700 }}>分组管理</Typography>
          ...
          <ListItem
            ...
            secondaryAction={(
              <Stack direction="row" spacing={0.5}>
                <Tooltip title="上移">
                  <span>
                    <IconButton disabled={index === 0} onClick={() => setGroups(prev => move(prev, index, -1))}>
                ...
                <Tooltip title="下移">
                  <span>
                    <IconButton disabled={index === groups.length - 1} onClick={() => setGroups(prev => move(prev, index, 1))}>
      ...
      <Dialog open={Boolean(editing)} onClose={() => setEditing(null)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          <Button variant="text" disabled={savingGroup} onClick={() => setEditing(null)}>{t('common.cancel')}</Button>
          <Button loading={savingGroup} onClick={handleSaveGroup}>{t('common.confirm')}</Button>
        </DialogActions>
      </Dialog>
    </Stack>
  )
